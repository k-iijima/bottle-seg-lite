...
リポジトリルートから実行します（compose ファイルはルートの `docker-compose-cvat.yml`、
環境変数はルートの `.env` が自動で読まれます）:
...
```bash
make cvat-superuser # = docker exec -it cvat_server python3 ~/manage.py createsuperuser
```
...
（ポートを変える場合は `CVAT_PORT` も設定。既定は 4810。
このマシンでは 8080 が WSL の code-server に占有されているため使わないこと）

その後、再起動します。

```bash
make cvat-down && make cvat-up
```
...
```bash
docker compose -f docker-compose-cvat.yml down -v
```
...
```bash
make cvat-logs      # = docker compose -f docker-compose-cvat.yml logs -f cvat_server
```
