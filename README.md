...
- **10属性の推定**: 追跡中のボトルには 2 段目の軽量分類器（MobileNetV3-Small, 5MB,
  `assets/models/attr_cls.onnx`）で材質/蓋/色/中身/変形等の 10 属性を推定し、
  パネルに日本語表示します。クロップは元解像度フレームから低頻度（0.7〜1s/トラック）で
  切り出し、トラック内で softmax を EMA 集約（確信度不足は `?` 表示。2値属性は閾値 0.65）。
  蓋・ラベルの有無は検出器の cap/label パーツ検出も証拠として融合。教師が Qwen3-VL の
...
- `attr_classifier.dart`
  - 追跡中ボトルのクロップ（元解像度から bbox+15% pad、長辺 96px 以上のみ）を
    128×128 で推論し、トラック内で softmax を EMA 集約。蓋・ラベル有無は検出器の
