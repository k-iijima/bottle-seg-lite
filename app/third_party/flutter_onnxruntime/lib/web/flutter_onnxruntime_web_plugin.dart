...
class FlutterOnnxruntimeWebPlugin extends FlutterOnnxruntimePlatform {
  ...
  JSObject _convertToTypedArray(dynamic data, String arrayType) {
    // LOCAL PATCH: TypedData は .toJS で直接 JS TypedArray に渡せる。
    // 中間の JS Array 生成 + TypedArray.from コピー（要素数に比例する
    // interop 呼び出し）を避ける。realtime 用途では支配的コストだった。
    if (arrayType == 'Float32Array' && data is typed_data.Float32List) {
      return data.toJS;
    }
    if (arrayType == 'Int32Array' && data is typed_data.Int32List) {
      return data.toJS;
    }
    if (arrayType == 'Uint8Array' && data is typed_data.Uint8List) {
      return data.toJS;
    }
  ...
  Future<Map<String, dynamic>> getOrtValueData(String valueId) async {
    try {
      ...
      switch (type.toString()) {
        case 'float32':
          // LOCAL PATCH: JS Float32Array を一括変換（旧実装は 1 要素ずつ
          // getProperty(i.toString()) しており、マスク等の大きいテンソルで
          // 100ms 超かかっていた）。
          data = (jsData as JSFloat32Array).toDart;
          break;

        case 'int32':
          // LOCAL PATCH: 一括変換（上記 float32 と同様）。
          data = (jsData as JSInt32Array).toDart;
          break;
        ...
        case 'uint8':
          // LOCAL PATCH: 一括変換（上記 float32 と同様）。
          data = (jsData as JSUint8Array).toDart;
          break;
